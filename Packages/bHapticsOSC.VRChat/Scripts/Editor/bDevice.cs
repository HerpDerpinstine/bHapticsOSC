    ...
    public static class bDevice
    {
        ...
        static bDevice()
        {
            ...
            foreach (bDeviceTemplate settings in AllTemplates.Values)
            {
                ...
                string withoutMeshStr = $"Assets/bHapticsOSC.VRChat/Resources/Prefabs/Without Mesh/{nameWithoutSpaces}.prefab";
                if (File.Exists(withoutMeshStr))
                    settings.Prefab = (GameObject)EditorGUIUtility.Load(withoutMeshStr);

                string withMeshStr = $"Assets/bHapticsOSC.VRChat/Resources/Prefabs/With Mesh/{nameWithoutSpaces}.prefab";
